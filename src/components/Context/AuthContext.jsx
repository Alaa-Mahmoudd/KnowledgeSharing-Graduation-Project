...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = () => {
        ...
        if (token) {
            try {
                ...
                console.log('Decoded token data:', decodedData); // Debug log
    ...
    const handleLogin = (token) => {
        try {
            const decodedData = jwtDecode(token);
            console.log('Raw decoded token:', decodedData); // Debug log
            ...
            // If we have a name in the token, update localStorage
            if (userData.name) {
                localStorage.setItem('userName', userData.name);
            }

            console.log('Final user data being set:', userData); // Debug log
            setUserData(userData);
            setIsAuthenticated(true);
            navigate('/');
        } catch (error) {
